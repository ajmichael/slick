...
char **argv;
{
	...
	best[2][1]=best[1][2];
	fgets(line,100,fpin);
	sscanf(line,"%f %f %f %f %f %f %f",&phi,&t1,&d1,&t2,&d2,&t3,&d3);
	...
	for(j=0;j<i;++j){
		...
		stress[2][1]=stress[1][2];
		fgets(line,100,fpin);
		sscanf(line,"%f %f %f %f %f %f %f",&phi,&tr1[k],&pl1[k],&tr2[k],&pl2[k],
		&tr3[k],&pl3[k]);
		tenmag(stress,&mag);
	...
	fprintf(fpout,"symbol line 1 1\n");
	draw_far(t1,d1,tr1,pl1,k,fpout);
	fprintf(fpout,"symbol line 2 2\n");
	draw_far(t2,d2,tr2,pl2,k,fpout);
	fprintf(fpout,"symbol line 3 3\n");
	draw_far(t3,d3,tr3,pl3,k,fpout);
}
