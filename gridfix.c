/* version with 1-cos(beta) error condition */
#include <math.h>
#include <stdio.h>
#include "controls"
...
char **argv; /* argument string */
{
	...
	++argv;
	if(argc == 0){
		printf("usage: gridfix data_file\n");
		return;
	}
	fpin=fopen(*argv,"r");
	if(fpin==NULL){
	...
	sprintf(name,"%s.goput",*argv);
	sprintf(line,"cat controls > %s",name);
	system(line);
	fpout=fopen(name,"a");
	if(fpout==NULL){
		printf("unable to open %s.\n",name);
	...
	while(fscanf(fpin,"%f%f%f%d",&ddir[nobs],&dip[nobs],&rake[nobs],&set[nobs])
	    != EOF )
	{
		fprintf(fpout,"%6.1f %6.1f %6.1f %4d\n",
		ddir[nobs],dip[nobs],rake[nobs],set[nobs]);
		i=nobs;
		j=6*nobs;
		++nobs;
		z=ddir[i]/TORADS;
		...
		for(beta= BETASTART;beta<= BETASTOP;beta+= BETASTEP){
			...
			for(gamma= GAMMASTART;gamma<= GAMMASTOP;gamma+= GAMMASTEP){
				...
				for(phi=PHISTART; phi<=PHISTOP; phi+= PHISTEP){
					...
					best[3]=gamma;
					best[4]=phi;
					for(i=0;i<nobs;++i)bestwh[i]=which[i];
					first=0;
					fprintf(fpout,"%6f %4.0f %4.0f %4.0f %4.2f\n",
					error,theta,beta,gamma,phi);
					for(i=0;i<nobs;++i){
						fprintf(fpout,"%c",bestwh[i]);
						if((i+1)%80==0)fprintf(fpout,"\n");
					}
					fprintf(fpout,"\n");
	...
	for(i=0;i<nobs;++i)fprintf(fpout,"%c\n",bestwh[i]);
}
